...
int WriteCenterline(vtkPolyData* centerline, string centerlinePath)
{
			...
			centerlinePathInfo.suffix().toStdString() == "VTK"))
	{
		...
		branchExtractor->SetInputData(attributeFilter->GetOutput());
...
int ExtractCenterline(string surfacePath, vtkPolyData* inputSurface, vtkPolyData* centerline, bool &cylindricalShape)
{
	QFileInfo inputSurfaceInfo(surfacePath.c_str());
	...
	if (inputSurfaceInfo.exists())
	{
		...
		else
		{
			...
			surfaceMapper->SetInputData(capper->GetOutput());
			...
			style->SetSurface(capper->GetOutput());
...
int main(int argc, char *argv[])
{
	if (argc < 2)
	{
		cout << "Usage: Centerline.exe surface.stl centerline.vtp" << endl;
		return 0;
	}
	else
	{
		...
		if (!ExtractCenterline(argv[1], inputSurface,centerline, cylindricalShape))
			return 0;
		else
		{
			...
			if (argc > 2)
			{
				if (!WriteCenterline(centerline, argv[2]))
					return 0;
			}
		}
		//	

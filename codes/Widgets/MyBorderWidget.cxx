...
void MyBorderWidget::SetLowerLeft(double x, double y, double z)
{
	...
	double viewPoint[3];
	renderer->GetViewPoint(viewPoint);
	renderer->ViewToNormalizedViewport(viewPoint[0], viewPoint[1], viewPoint[2]);
	//GetBorderRepresentation()->GetPositionCoordinate()->
	//	SetValue(viewPoint[0], viewPoint[1], viewPoint[2]);
	GetBorderRepresentation()->SetPosition(viewPoint[0], viewPoint[1]);
}
...
void MyBorderWidget::SetUpperRight(double x, double y, double z)
{
	...
	const double* point = GetBorderRepresentation()->GetPosition();
	GetBorderRepresentation()->SetPosition2(viewPoint[0] - point[0],
		viewPoint[1] - point[1]);

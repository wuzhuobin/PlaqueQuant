 ...
 *
 *=========================================================================*/
#ifndef __IADE_HOUGH_TRANSFORM_2D_CIRCLIES_IMAGE_FILTER_HXX__
#define __IADE_HOUGH_TRANSFORM_2D_CIRCLIES_IMAGE_FILTER_HXX__
...
IADEHoughTransform2DCirclesImageFilter< TInputPixelType, TOutputPixelType, TMaskPixelType >
...
::GenerateData()
{
  ...
  while ( !image_it.IsAtEnd() )
    ...
    if ( image_it.Get() > m_Threshold )
	  ...
	  typename TMaskPixelType pixel = m_MaskImage->GetPixel(image_it.GetIndex());

      // if the gradient is not flat
      if( ( ( std::fabs(Vx) > 1 ) || ( std::fabs(Vy) > 1 ) ) && (pixel != 0))
        {
...
template< typename TInputPixelType, typename TOutputPixelType, typename TMaskPixelType >
typename IADEHoughTransform2DCirclesImageFilter< TInputPixelType, TOutputPixelType, TMaskPixelType >::CirclesListType &
IADEHoughTransform2DCirclesImageFilter< TInputPixelType, TOutputPixelType, TMaskPixelType >

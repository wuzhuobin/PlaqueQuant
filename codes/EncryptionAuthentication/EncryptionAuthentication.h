...
public:
	...
	~EncryptionAuthentication();
	/**
	 * set error message of the m_messageBox @see #m_messageBox
	 ...
	 * before it check USB key, it will always call reloadVikey @see #reloadVikey() bofore
	 * if fail the authentication, it will always pop up a QMessageBox::Critical messagebox
	 * with specifed messages in @see #m_messages, only the failed requested message will 
	 * pop up
	...
	/**
	 * @personal deprecated
	 * almost the same as authenticationExec @see #authenticationExec
	 ...
	 * @param	true, if pass the authentication, it will always pop up a hint
	 *			false, no hint will pop up
	 */
